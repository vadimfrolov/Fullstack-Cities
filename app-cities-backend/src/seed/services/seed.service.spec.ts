...
describe('SeedService', () => {
  ...
  beforeEach(async () => {
    ...
    jest.spyOn(global, 'setTimeout').mockImplementation((callback: any, _timeout?: any) => {
      callback();
  ...
  it('should handle database connection error', async () => {
    jest.spyOn(repository, 'query').mockRejectedValue(new Error('DB connection failed'));
  ...
  it('should read cities from production path', async () => {
    ...
    jest.spyOn(repository, 'create').mockImplementation((entity) => entity as CityEntity);
    jest.spyOn(repository, 'save').mockResolvedValue(mockCities as unknown as CityEntity);
    
    await (service as any).seedCities();
    
    expect(fs.readFileSync).toHaveBeenCalledWith('/mock/prod/path/cities.json', 'utf8');
  ...
  it('should fall back to development path if production path fails', async () => {
    ...
    (path.join as jest.Mock).mockImplementation((...args) => {
      if (args[0] === process.cwd() && args[1] === 'cities.json') {
        return mockProdPath;
      } else if (args[0] === '__dirname' || args[1] === '../../../cities.json') {
        return mockDevPath;
      }
      return jest.requireActual('path').join(...args);
    });
  ...
  it('should use hardcoded sample data if both paths fail', async () => {
    ...
    jest.spyOn(repository, 'create').mockImplementation((entity) => entity as CityEntity);
    jest.spyOn(repository, 'save').mockResolvedValue([{}, {}] as unknown as CityEntity);
  ...
  it('should handle errors during seeding process', async () => {
    ...
    expect(consoleErrorSpy).toHaveBeenCalledWith('Error seeding cities:', 'Save failed');
  });
});
